...
/// <summary>
/// Tests for runtime compilation features with FluentPartitionModels that require Select/expression compilation
/// These tests are skipped when PublishAot is enabled
/// </summary>
public class FluentPartitionRuntimeCompilationTests(AzuriteFixture azuriteFixture) : AzuriteTestBase(azuriteFixture)
{
    [Fact]
    public async Task Select_WithAnonymousType_ShouldProjectProperties()
    {
        // Arrange
        await Context.FluentPartitionModels.UpsertEntityAsync(new FluentTestModelA()
        {
    ...
    public async Task BatchUpdateAsync_WithExpressionIncrement_ShouldIncrementValue()
    {
        ...
        await Context.FluentPartitionModels.UpsertEntityAsync(fluentModel, TestContext.Current.CancellationToken);
    ...
    public async Task UpdateAsync_ShouldUpdateEntity()
    {
        ...
        await Context.FluentPartitionModels.UpsertEntityAsync(model, TestContext.Current.CancellationToken);

        // Act
        await Context.FluentPartitionModels.UpdateAsync(() => new FluentTestModelA
        {
            PrettyRowA = model.PrettyRowA,
            TypeA = "after",
            PropertyA = 2
        }, TestContext.Current.CancellationToken);

        // Assert
        var updated = await Context.FluentPartitionModels
            .WhereFluentTestModelA(x => x.PrettyRowA == model.PrettyRowA)
            .FirstOrDefaultAsync(TestContext.Current.CancellationToken);
    ...
    public async Task UpsertAsync_ShouldInsertOrUpdateEntity()
    {
        ...
        // Act
        await Context.FluentPartitionModels.UpsertAsync(() => new FluentTestModelA
        {
            PrettyRowA = model.PrettyRowA,
            TypeA = model.TypeA,
        ...
        // Assert
        var upserted = await Context.FluentPartitionModels
            .WhereFluentTestModelA(x => x.PrettyRowA == model.PrettyRowA)
            .FirstOrDefaultAsync(TestContext.Current.CancellationToken);
