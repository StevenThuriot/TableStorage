...
/// <summary>
/// Tests for runtime compilation features with FluentModels that require Select/expression compilation
/// These tests are skipped when PublishAot is enabled
/// </summary>
public class FluentRuntimeCompilationTests(AzuriteFixture azuriteFixture) : AzuriteTestBase(azuriteFixture)
{
    [Fact]
    public async Task Select_WithAnonymousType_ShouldProjectProperties()
    {
        // Arrange
        await Context.FluentModels.UpsertEntityAsync(new FluentTestModelA()
        {
    ...
    public async Task BatchUpdateAsync_WithExpressionIncrement_ShouldIncrementValue()
    {
        ...
        int mergeCount = await Context.FluentModels
            .Where((FluentTestModelA x) => x.PrettyPartitionA == fluentModel.PrettyPartitionA && x.PrettyRowA == fluentModel.PrettyRowA)
